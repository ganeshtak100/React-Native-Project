...
import {ScrollbarItemProps, ScrollbarProps} from '../src/types';
...
export const Scrollbar = ({
  ...
  const gestureHandler = useAnimatedGestureHandler({
    onStart: (event, ctx) => {
      ...
      scrollIndex.value = withTiming(getSectionFromScrollY(event.y), {
        duration: 500,
...
const ScrollbarItem = ({section, index, scrollIndex}: ScrollbarItemProps) => {
  ...
  const animatedTextStyles = useAnimatedStyle(() => {
    return {
      ...
      transform: [{scale: withSpring(active.value ? 2 : 1)}],
    };
  });

  return (
    <Animated.View style={[styles.scrollbarItem, animatedViewStyles]}>
      {/* <Animated.Text style={[styles.scrollbarItemText, animatedTextStyles]}>
        {title}
      </Animated.Text>
...
//   withSpring,
// } from 'react-native-reanimated';
// import {ScrollbarItemProps} from '../src/types';
// import {ScrollbarIndicator} from './ScrollbarIndicator';
